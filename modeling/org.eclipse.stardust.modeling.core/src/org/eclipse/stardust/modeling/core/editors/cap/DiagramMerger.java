...
{   
   ...
   {      
      ...
      editor = (WorkflowModelEditor) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor();      
      isDiagram = true;
      storage.setIsDiagram(true);

      DiagramEditorPage diagramEditorPage = (DiagramEditorPage) editor.getCurrentPage();
      targetDiagram = (DiagramType) diagramEditorPage.getDiagram();      
   ...
   {      
      ...
      if(storage.getTargetEditPart() != null)
      {
         coordinatesDifference = getLocationDifference();               
      }
   ...
   private Dimension getLocationDifference()
   {
      ...
      Point viewPortPoint = PoolLaneUtils.findViewportPoint((GraphicalEditPart) storage.getTargetEditPart()); 
      newMouseLocation.performTranslate(viewPortPoint.x, viewPortPoint.y);
      // difference between old mouse location and new mouse location
      Dimension dimension = storage.getLocation().getDifference(newMouseLocation);
      return dimension;
   }
   ...
   protected void createSymbol(Entry symbolEntry, boolean onlyChange)
   {
      ...
      // mouse location is absolute
      Point newMouseLocation = diagramEditorPage.getMouseLocation();
      // the old symbol location (absolue)
      Point location = new Point(((INodeSymbol) modelSymbol).getXPos(), ((INodeSymbol) modelSymbol).getYPos());         
      // new location in the target
      Point newContainerLocation;
      if(storage.getTargetEditPart() != null)
      {
         newContainerLocation = PoolLaneUtils.getLocation((GraphicalEditPart) storage.getTargetEditPart(), 
               ((AbstractGraphicalEditPart) storage.getTargetEditPart()).getFigure(), 
               newMouseLocation, true);         
         
         // difference between old mouse location and new mouse location
         Dimension dimension = storage.getLocation().getDifference(location);  
         // new location for the symbol
      ...
      if(rawSymbol instanceof IModelElementNodeSymbol)
      {
         IIdentifiableModelElement rawModelElement = ((IModelElementNodeSymbol) rawSymbol).getModelElement();
         if(rawModelElement != null)
         {                      
            EObject parent = rawModelElement.eContainer();
            // if the model has more than one process we need the correct target process
            ...
            // check symbol later for gateways
            if(modelElement instanceof ActivityType)
            {
               checkActivities.add(modelSymbol);
            }
         }         
      }      
      ...
      if(parentLane == null)
      {
         ((INodeSymbol) modelSymbol).setXPos(newContainerLocation.x);
         ((INodeSymbol) modelSymbol).setYPos(newContainerLocation.y);            
      }      
      
      ISymbolContainer container = (ISymbolContainer) storage.getTargetObject();
      // the real container is the pool then
      if(modelSymbol instanceof LaneSymbol)
      {
         container = DiagramUtil.getDefaultPool(diagramEditorPage.getDiagram());
      }      
      ...
      if(modelSymbol instanceof LaneSymbol)
      {
         if(parentLane != null)
         {
            ((LaneSymbol) modelSymbol).setParentLane(parentLane);            
         }
         else
         {
            ((LaneSymbol) modelSymbol).setParentLane((ISwimlaneSymbol) storage.getTargetEditPart().getModel());            
         }
