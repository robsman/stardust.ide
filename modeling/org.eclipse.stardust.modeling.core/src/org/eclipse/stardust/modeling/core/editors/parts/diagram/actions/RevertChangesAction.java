...
import org.eclipse.stardust.modeling.core.modelserver.ui.UnlockDialog;
 ...
 */
public class RevertChangesAction extends SelectionAction
{
   public RevertChangesAction(WorkflowModelEditor editor)
   {
      super(editor);
      setText(Diagram_Messages.LB_REVERT_CHANGES);
      setToolTipText(Diagram_Messages.LB_REVERT_CHANGES);
      setId(DiagramActionConstants.REVERT_CHANGES);      
      setImageDescriptor(DiagramPlugin.getImageDescriptor("icons/full/obj16/revert.gif")); //$NON-NLS-1$
   }
   ...
   public void run()
   {
      ...
      // analyze revert
      final RevertUtil revertUtil = new RevertUtil(editor);
      ...
      if (revertUtil.getLockedElements().isEmpty())
      {
         ModelServerUtils.showMessageBox(Diagram_Messages.MSG_NO_LOCKS);
         return;
      }            
      ...
      for (Object object : selectedObjects)
      {
         ...
         {
            preCheckedElements.add(revertUtil.getLocalModel());
         }
         else
         {
            preCheckedElements.add(CopyPasteUtil.getSameElement(element, revertUtil.getLocalModel()));
         }
      }      
      EObject[] lockables = preCheckedElements.toArray(new EObject[preCheckedElements.size()]);
      
      final UnlockDialog dialog = new UnlockDialog(revertUtil);
      ...
      IRunnableWithProgress op = new IRunnableWithProgress()
      {
               ...
               InterruptedException
         {
            try
            {
               editor.getModelServer().revert(revertUtil, monitor);
            }
            catch (RMSException e)
            {
               throw new InvocationTargetException(e);
