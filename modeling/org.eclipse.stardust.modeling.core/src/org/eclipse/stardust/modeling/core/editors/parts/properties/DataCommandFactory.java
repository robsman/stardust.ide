...
public class DataCommandFactory extends DefaultPropSheetCmdFactory
{
                                 ...
                                 IPropertySource target, Object value)
   {
      ...
      if (feature.equals(CWM_PKG.getDataType_Type()))
      {
         addResetCommands(result, (DataType) target.getEditableValue(),
            (DataTypeType) value);
      }
   }
   ...
   {
      List<AttributeType> attributes = data.getAttribute();
      ...
      IDataInitializer initializer = getInitializer(type);
      if (initializer != null)
      {
   ...
   private IDataInitializer getInitializer(DataTypeType type)
   {
      if (type != null)
      {
         ...
         Map extensions = registry.getExtensions(CarnotConstants.DATA_TYPES_EXTENSION_POINT_ID);
         IConfigurationElement config = (IConfigurationElement) extensions.get(type.getId());
         if (config != null)
         {
            try
            {
               return (IDataInitializer)
   ...
   public static Command getSetTypeCommand(DataTypeType type, DataType data)
   {
      ...
      return INSTANCE.getSetCommand(null, data,
             INSTANCE.CWM_PKG.getDataType_Type(), type);
   }
}
