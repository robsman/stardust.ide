 ...
 *     Sungard         - hooked in a customized Java code scanner
 *                     - hooked in a customized editor configuration
 *                     - added accessor methods for private attributes
 *                     - added some new attributes and accessor methods
 *******************************************************************************/
 ...
 */
public class JSCompilationUnitEditor extends JavaEditor implements IJavaReconcilingListener {
    ...
    private AdaptedSourceViewer adaptedSourceViewer;
	...
	public JSCompilationUnitEditor(AbstractJavaScanner codeScanner)
    {
       this();
    ...
    public class AdaptedSourceViewer extends JavaSourceViewer implements KeyListener {
       ...
       public AdaptedSourceViewer(Composite parent, IVerticalRuler verticalRuler, IOverviewRuler overviewRuler, boolean showAnnotationsOverview, int styles, IPreferenceStore store) {
             super(parent, verticalRuler, null, showAnnotationsOverview, styles, store);         
             if (theSite != null) 
             {                 
                 ...
                 getTextWidget().addExtendedModifyListener(new ExtendedModifyListener() 
                 {
                    public void modifyText(ExtendedModifyEvent event) {
                       if (!undone) {
                           UndoRedoObject undoRedo = new UndoRedoObject(event);
                           undoStack.add(0, undoRedo); 
                       } 
                       undone = false;
                    }
                 });
         ...
         public void keyPressed(KeyEvent arg0) {
             if (theSite != null) {
                 if (this.getTextWidget().isFocusControl()) {
   ...
   private SandboxEditorSite theSite;
private DeleteNextSubWordAction deleteWordAction;
private EditorSourceViewerConfiguration sourceViewerConfiguration;
     ...
     */
    public JSCompilationUnitEditor() {
        super();
    ...
    protected void createActions() {
        ...
        /*IAction action= new ContentAssistAction(EditorMessages.getBundleForConstructedKeys(), "ContentAssistProposal.", this); //$NON-NLS-1$
        ...
        action= new TextOperationAction(EditorMessages.getBundleForConstructedKeys(), "ContentAssistContextInformation.", this, ISourceViewer.CONTENTASSIST_CONTEXT_INFORMATION);   //$NON-NLS-1$
        ...
        action= new TextOperationAction(EditorMessages.getBundleForConstructedKeys(), "Comment.", this, ITextOperationTarget.PREFIX); //$NON-NLS-1$
        ...
        action= new TextOperationAction(EditorMessages.getBundleForConstructedKeys(), "Uncomment.", this, ITextOperationTarget.STRIP_PREFIX); //$NON-NLS-1$
        ...
        action= new ToggleCommentAction(EditorMessages.getBundleForConstructedKeys(), "ToggleComment.", this); //$NON-NLS-1$
        ...
        action= new TextOperationAction(EditorMessages.getBundleForConstructedKeys(), "Format.", this, ISourceViewer.FORMAT); //$NON-NLS-1$
        ...
        action= new AddBlockCommentAction(EditorMessages.getBundleForConstructedKeys(), "AddBlockComment.", this);  //$NON-NLS-1$
        ...
        action= new RemoveBlockCommentAction(EditorMessages.getBundleForConstructedKeys(), "RemoveBlockComment.", this);  //$NON-NLS-1$
        ...
        action= new IndentAction(EditorMessages.getBundleForConstructedKeys(), "Indent.", this, false); //$NON-NLS-1$
        ...
        action= new IndentAction(EditorMessages.getBundleForConstructedKeys(), "Indent.", this, true); //$NON-NLS-1$
        setAction("IndentOnTab", action); //$NON-NLS-1$
        markAsStateDependentAction("IndentOnTab", true); //$NON-NLS-1$
        markAsSelectionDependentAction("IndentOnTab", true); //$NON-NLS-1$*/
        
		deleteWordAction = new DeleteNextSubWordAction();
		
	    MenuManager menuMgr = new MenuManager();
	    Menu menu = menuMgr.createContextMenu(getAdaptedSourceViewer().getTextWidget());
	    menuMgr.addMenuListener(new IMenuListener() {
	       public void menuAboutToShow(IMenuManager manager) {
	           addPopupAction(manager);
	       }
	    });
	    menuMgr.setRemoveAllWhenShown(true);
	    getAdaptedSourceViewer().getTextWidget().setMenu(menu);
        
        // override the text editor actions with indenting move line actions
        /*JavaMoveLinesAction[] moveLinesActions= JavaMoveLinesAction.createMoveCopyActionSet(EditorMessages.getBundleForConstructedKeys(), this);
...
//		manager.add(removeBlockCommentAction);	
	}    
    ...
    protected ISourceViewer createJavaSourceViewer(Composite parent, IVerticalRuler verticalRuler, IOverviewRuler overviewRuler, boolean isOverviewRulerVisible, int styles, IPreferenceStore store) {
       adaptedSourceViewer = new AdaptedSourceViewer(parent, verticalRuler, overviewRuler, isOverviewRulerVisible, styles, store);
       //adaptedSourceViewer.configure(messagingConfiguration);
    ...
    public void aboutToBeReconciled() {
        ...
        // Notify AST provider
        //Todo: Removed line from original
	...
	{
	   adjustableLineNumberRuler = new AdjustableLineNumberRuler();
      return adjustableLineNumberRuler;
	}
   ...
   public void setTheSite(IWorkbenchPartSite site) {
       theSite = new SandboxEditorSite((IEditorSite) site);
   }
    
    private void undo() {
       if (undoStack.size() > 0) {
          undone = true;  
          String currentText = adaptedSourceViewer.getTextWidget().getText();
          ...
          undoRedo.setAdded(!undoRedo.isAdded());
          redoStack.add(0, undoRedo);
        }
     }
 	...
 	protected JavaScriptSourceViewerConfiguration createJavaSourceViewerConfiguration() {
		...
		sourceViewerConfiguration = new EditorSourceViewerConfiguration(textTools.getColorManager(), getPreferenceStore(), this, IJavaScriptPartitions.JAVA_PARTITIONING, codeScanner);	
		return sourceViewerConfiguration;
	}
     
     public IColorManager getUsableColorManager() {
        JavaScriptTextTools textTools= JavaScriptPlugin.getDefault().getJavaTextTools();
        return textTools.getColorManager();
     }
     
     public IPreferenceStore getUsablePreferenceStore() {
        return this.getPreferenceStore();
     }    
     
     public ProjectionViewer getAdaptedSourceViewer() {
        return adaptedSourceViewer;
     }
   ...
   protected void setPreferenceStore(IPreferenceStore store)
   {
     ...
     {
        if (null != adjustableLineNumberRuler)
        {
           adjustableLineNumberRuler.setLineOffset(lineOffset);
        }
     }
     ...
     public int getLineOffset()
     {
        if (null != adjustableLineNumberRuler)
        {
           return adjustableLineNumberRuler.getLineOffset();
        }
     ...
     /**
      * Used to show line numbers in editor. But these line numbers are modified by an offset.
      * This is necessary because the editor shows only a subset of lines.
      *
      * @author born
      */
     private static final class AdjustableLineNumberRuler extends LineNumberRulerColumn
    {
       ...
       {
             return super.createDisplayString(line - lineOffset);
       }
     ...
     /**
      * This Region implementation additionally stores the offset as line.
      *  
      * @author born
      */
     public static final class RegionWithLineOffset extends Region
     {
        private int lineOffset = 0;

        public RegionWithLineOffset(int offset, int length)
       {
          super(offset, length);
       }
