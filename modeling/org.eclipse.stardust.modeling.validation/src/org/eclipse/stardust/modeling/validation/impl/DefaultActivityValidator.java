         ...
         org.eclipse.stardust.model.xpdl.xpdl2.LoopType loop)
   {
      ...
      else
      {
         ...
         case STANDARD:
            ...
            break;
         case MULTI_INSTANCE:
            validateLoopMulti(result, activity, loop.getLoopMultiInstance());
            break;
         default:
            result.add(Issue.error(activity,
                  Validation_Messages.ERR_ACTIVITY_UndefinedStandardLoop,
                  XpdlPackage.eINSTANCE.getLoopType_LoopType()));
            break;
         }
      }
   }

   private void validateLoopMulti(List<Issue> result, ActivityType activity,
         LoopMultiInstanceType loopMulti)
   {
      if (loopMulti == null)
      {
         if (activity.getLoop().getLoopStandard() == null)
         {
            result.add(Issue.error(activity,
               Validation_Messages.ERR_ACTIVITY_UndefinedStandardLoop,
               XpdlPackage.eINSTANCE.getLoopType_LoopMultiInstance()));
         }
         else
         {
            result.add(Issue.error(activity,
                  Validation_Messages.ERR_ACTIVITY_InvalidMultiInstanceLoop,
                  XpdlPackage.eINSTANCE.getLoopType_LoopMultiInstance()));
         }
      }
      else
      {
         ...
         {
            result.add(Issue.error(activity,
                  Validation_Messages.ERR_ACTIVITY_NoMultiInstanceLoopOrderingType,
                  XpdlPackage.eINSTANCE.getLoopMultiInstanceType_MIOrdering()));
         }
         else if (loopMulti.getMIOrdering() == MIOrderingType.PARALLEL
               && activity.getImplementation() == ActivityImplementationType.SUBPROCESS_LITERAL
               && activity.getSubProcessMode() == SubProcessModeType.SYNC_SHARED_LITERAL)
         {
            result.add(Issue.error(activity,
                  Validation_Messages.ERR_ACTIVITY_IncompatibleSubProcessMode,
                  ValidationService.PKG_CWM.getActivityType_SubProcessMode()));
         }
         if (loopMulti.getLoopDataRef() == null || StringUtils.isEmpty(loopMulti.getLoopDataRef().getInputItemRef()))
         {
            result.add(Issue.error(activity,
                  Validation_Messages.ERR_ACTIVITY_NoInputLoopDataSpecified,
         ...
         LoopStandardType loopStandard)
   {
      if (loopStandard == null)
      {
         if (activity.getLoop().getLoopMultiInstance() == null)
         {
            result.add(Issue.error(activity,
               Validation_Messages.ERR_ACTIVITY_UndefinedStandardLoop,
               XpdlPackage.eINSTANCE.getLoopType_LoopStandard()));
         }
         else
         {
            result.add(Issue.error(activity,
      ...
      else
      {
         ...
         {
            result.add(Issue.error(activity,
                  Validation_Messages.ERR_ACTIVITY_NoStandardLoopExecutionType,
                  XpdlPackage.eINSTANCE.getLoopStandardType_TestTime()));
         }
