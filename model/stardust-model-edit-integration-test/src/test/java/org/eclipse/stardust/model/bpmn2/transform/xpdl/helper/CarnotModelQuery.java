...
//        	String eventPathRouteId = BoundaryEvent2Stardust.getBoundaryEventEventPathRouteId(activity);
    ...
    public static ActivityType findGateway(ProcessDefinitionType processDef, String id) {
        return findActivity(processDef, "gateway" + id);
    }
	...
	public static DataTypeType getMetaDataType(ModelType model, String typeRef) {
		if (typeRef == null || typeRef.isEmpty()) return null;
		return (DataTypeType)
				ModelUtils.findIdentifiableElement(model, CarnotWorkflowModelPackage.eINSTANCE.getModelType_DataType(), typeRef);
	}
