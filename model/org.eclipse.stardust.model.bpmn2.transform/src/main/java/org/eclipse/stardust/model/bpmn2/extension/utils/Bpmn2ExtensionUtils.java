package org.eclipse.stardust.model.bpmn2.extension.utils;
...
public class Bpmn2ExtensionUtils
{
   ...
   public static List<EObject> getExtensionElements(BaseElement object, String tag, String nsUri)
   {
      ...
      if (null != extensionAttributes)
      {
         for (FeatureMap.Entry extension : extensionAttributes.getValue())
         {
            if (isInFilter(extension.getEStructuralFeature(), tag, nsUri))
            {
               ...
               if (extensionValue instanceof EObject)
               {
                  result.add((EObject) extensionValue);
               }
            }
         }
      }
